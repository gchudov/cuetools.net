...
namespace CUETools.DSP.Resampler.Internal
{
	class SOXFft
	{
		...
		static unsafe void makect(int nc, int* ip, double* c)
		{
			int j, nch;
			double delta;
		...
		static unsafe void makewt(int nw, int* ip, double* w)
		{
			int j, nwh;
			double delta, x, y;
		...
		static unsafe void cft1st(int n, double* a, double* w)
		{
			int j, k1, k2;
		...
		static unsafe void cftmdl(int n, int l, double* a, double* w)
		{
		...
		static unsafe void cftfsub(int n, double* a, double* w)
		{
			int j, j1, j2, j3, l;
			double x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i;
		...
		static unsafe void rftfsub(int n, double* a, int nc, double* c)
		{
			int j, k, kk, ks, m;
		...
		static unsafe void rftbsub(int n, double* a, int nc, double* c)
		{
			int j, k, kk, ks, m;
			double wkr, wki, xr, xi, yr, yi;
		...
		static unsafe void cftbsub(int n, double* a, double* w)
		{
			int j, j1, j2, j3, l;
			double x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i;
		...
		public static unsafe void rdft(int n, int isgn, double* a, int* ip, double* w)
		{
			int nw, nc;
			double xi;
		...
		static int dft_br_len(int l)
		{
			return 2 + (1 << (int)(Math.Log(l / 2 + .5) / Math.Log(2.0)) / 2);
		}

		static int dft_sc_len(int l)
		{
			return l / 2;
		}
